...
-- RPC para cotação de migração de plano (mensal -> anual à vista)
-- Conta pagamentos mensais aprovados nos últimos 12 meses e calcula
-- o valor restante para fechar o ciclo anual.
...
BEGIN
  ...
  -- Permissão: admin OU usuário aprovado da própria escola
  IF NOT (
  ...
  -- Início do ciclo: primeiro pagamento aprovado nos últimos 12 meses,
  -- caindo para "now() - 12 meses" se não houver.
  ...
  -- Conta pagamentos mensais aprovados dentro do ciclo
  SELECT COUNT(*)::int
    INTO v_pagos
  FROM pagamentos
  WHERE pagamentos.school_id = v_school
