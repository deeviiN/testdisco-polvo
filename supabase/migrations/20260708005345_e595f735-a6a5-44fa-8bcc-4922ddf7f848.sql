...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'roster_id', rr.absent_roster_id,
           'absent_teacher_name', rr.absent_teacher_name,
           'period_number', rr.absent_period_number,
           'reason', rr.reason,
           'location', rr.note,
           'covering_roster_id', rr.covering_roster_id,
           'covering_teacher_name', rr.covering_teacher_name,
           'covering_nickname', tr.nickname,
           'covering_discipline', tr.discipline
         )), '[]'::jsonb)
    INTO _extras
  FROM public.room_reassignments rr
  LEFT JOIN public.teacher_roster tr ON tr.id = rr.covering_roster_id
  WHERE rr.school_id = _school_id
    AND rr.reassignment_date = _today
    AND rr.cancelled_at IS NULL
    AND rr.absent_roster_id IS NOT NULL;
