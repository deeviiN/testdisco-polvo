...
CREATE OR REPLACE FUNCTION public.get_painel_tv_data(_school_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  _weekday smallint := EXTRACT(DOW FROM (now() AT TIME ZONE 'America/Manaus'))::smallint;
  _periods jsonb;
  _roster jsonb;
  _presence jsonb;
  _reduced jsonb;
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('roster_id', roster_id, 'status', status)), '[]'::jsonb)
    INTO _presence
...
GRANT EXECUTE ON FUNCTION public.get_painel_tv_data(uuid) TO anon, authenticated;
