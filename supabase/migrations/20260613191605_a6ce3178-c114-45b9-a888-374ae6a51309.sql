...
BEGIN
  ...
  SELECT r.school_id, r.shift, r.start_time, r.end_time
    INTO v_school_id, v_shift, v_start_time, v_end_time
    FROM public.teacher_roster r
   WHERE r.id = COALESCE(NEW.roster_id, OLD.roster_id);

  -- Prefer the day's reduced schedule, then the normal period schedule, then roster times.
  IF v_period_number IS NOT NULL AND v_school_id IS NOT NULL AND v_shift IS NOT NULL AND v_presence_date IS NOT NULL THEN
    SELECT srd.start_time, srd.end_time
  ...
  IF v_start_time IS NULL OR v_end_time IS NULL OR v_presence_date IS NULL THEN
    RETURN COALESCE(NEW, OLD);
  END IF;

  v_period_start := (v_presence_date::timestamp + v_start_time) AT TIME ZONE 'America/Manaus';
  v_period_end := (v_presence_date::timestamp + v_end_time) AT TIME ZONE 'America/Manaus';

  -- Assistant roles can freely change only during the current period window.
  IF v_role IN ('assistente','assistente_alunos','secretario_escolar') THEN
    IF now() >= v_period_start AND now() < v_period_end THEN
      RETURN COALESCE(NEW, OLD);
    END IF;

    IF now() < v_period_start THEN
      RAISE EXCEPTION 'Este tempo ainda não começou (% %).',
        v_presence_date, v_start_time
