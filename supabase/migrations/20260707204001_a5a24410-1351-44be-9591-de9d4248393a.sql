...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('roster_id', absent_roster_id, 'period_number', absent_period_number, 'location', note)), '[]'::jsonb)
    INTO _extras
  FROM public.room_reassignments
  WHERE school_id = _school_id
    AND reassignment_date = _today
    AND reason = 'atividade_extra'
    AND cancelled_at IS NULL
    AND absent_roster_id IS NOT NULL;
